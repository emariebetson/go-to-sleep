...
export default async function AccountPage() {
  ...
  return <AppShell active="account">
    ...
    <section className="panel" style={{ marginTop: 20 }}><h2>Subscription</h2><p className="panel-intro">{paid ? `NearSleep Plus · ${account?.subscriptionStatus}` : account?.subscriptionStatus === "past_due" ? "Plus · payment needs attention" : "Free plan"} · {account?.creditsRemaining ?? 0} generation {(account?.creditsRemaining ?? 0) === 1 ? "credit" : "credits"} remaining</p>{canManageBilling ? <form action="/api/billing/portal" method="post"><button className="btn btn-secondary" type="submit">Manage billing with Stripe</button></form> : <Link className="btn btn-secondary" href="/pricing">View Plus plan</Link>}</section>
