...
export async function prepareLiveProductionCatalog(input: LiveCatalogPreparationInput, dependencies: Dependencies) {
  ...
  precondition(files.length === 14 && files[5]?.id === "0006_private_canary_observation" && files[6]?.id === "0007_private_tester_deployment_manifest" && files[7]?.id === "0008_cloud_sql_iam_database_usernames" && files[8]?.id === "0009_cloud_sql_verifier_identity_limit" && files[9]?.id === "0010_migration_schema_usage" && files[10]?.id === "0011_private_tester_activation_controller" && files[11]?.id === "0012_nearfamily_private_tester_decision" && files[12]?.id === "0013_nearfamily_decision_nonce_and_evidence" && files[13]?.id === "0014_release_policy_evidence_read", "migration-set-invalid");
