...
const SITES_WORKER_SCRIPT = "site---6a79f8a66eb4819198bb42a2b26addea";
...
type Readers = {
  sitesDeployment: { read(accountId: string): Promise<Observed> };
...
function activeWorkerDeployment(value: unknown): { scriptName: string; deploymentId: string; versionId: string; percentage: 100 } { if (!exactRecord(value, ["scriptName", "deploymentId", "versionId", "percentage"]) || value.scriptName !== SITES_WORKER_SCRIPT || !text(value.deploymentId, UUID) || !text(value.versionId, UUID) || value.percentage !== 100) throw new Error("private tester baseline invalid"); return { scriptName: SITES_WORKER_SCRIPT, deploymentId: value.deploymentId, versionId: value.versionId, percentage: 100 }; }
...
export async function capturePrivateTesterBaseline(input: PrivateTesterBaselineInput): Promise<PrivateTesterBaseline> {
  ...
  if (manifest.releaseId !== release.releaseId || manifest.live.version !== release.sitesVersion || manifest.live.commitSha !== release.commitSha || deployment.projectId!==manifest.projectId || deployment.versionId!==manifest.live.version || deployment.commitSha!==manifest.live.commitSha || Math.abs(Date.parse(deployment.deployedAt)-manifest.issuedAt)>MAX_AGE_MS) throw new Error("private tester baseline invalid");
  ...
  const accountId = manifest.resources[0].resource.split("/")[1];
  if (!text(accountId, /^[a-f0-9]{32}$/)) throw new Error("private tester baseline invalid");
  const deploymentBeforeRaw = observation(await read.sitesDeployment.read(accountId), startedAt), deploymentBefore = activeWorkerDeployment(deploymentBeforeRaw.body);
  ...
  const deploymentAfterValue = await read.sitesDeployment.read(accountId);
  ...
  const deploymentAfterRaw = observation(deploymentAfterValue, capturedAt), deploymentAfter = activeWorkerDeployment(deploymentAfterRaw.body);
  const providerIdentity = `account:${accountId}`;
  if (deploymentBeforeRaw.meta.provider !== "cloudflare-api" || deploymentAfterRaw.meta.provider !== "cloudflare-api" || deploymentBeforeRaw.meta.identity !== providerIdentity || deploymentAfterRaw.meta.identity !== providerIdentity || canonical(deploymentBefore) !== canonical(deploymentAfter) || deploymentBefore.deploymentId !== deployment.workerDeploymentId || deploymentBefore.versionId !== deployment.workerVersionId) throw new Error("private tester baseline invalid");
  ...
  const d1LedgerRaw = observation(raw[0], capturedAt), d1SchemaRaw = observation(raw[1], capturedAt), pgMigrationsRaw = observation(raw[2], capturedAt), pgCatalogRaw = observation(raw[3], capturedAt), providerRaw = observation(raw[4], capturedAt), dnsRaw = observation(raw[5], capturedAt), oauthRaw = observation(raw[6], capturedAt), versionsRaw = observation(raw[7], capturedAt), gatesRaw = observation(raw[8], capturedAt);
  ...
  if (d1SchemaRaw.meta.identity !== d1LedgerRaw.meta.identity || oauthRaw.meta.identity !== d1LedgerRaw.meta.identity || gatesRaw.meta.identity !== d1LedgerRaw.meta.identity) throw new Error("private tester baseline invalid");
...
function cloudflareDeploymentReader(environment: NodeJS.ProcessEnv, fetcher: typeof fetch, now: () => number) {
  const apiToken = envId(environment, "CLOUDFLARE_API_TOKEN", /^[A-Za-z0-9._~-]{20,512}$/);
  return { read: async (accountId: string): Promise<Observed> => {
    if (!text(accountId, /^[a-f0-9]{32}$/)) throw new Error("private tester baseline reader unavailable");
    const response = await json(`https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/scripts/${SITES_WORKER_SCRIPT}/deployments`, apiToken, fetcher), result = response.result;
    if (response.success !== true || !Array.isArray(response.errors) || response.errors.length !== 0 || !Array.isArray(response.messages) || !exactRecord(result, ["deployments"]) || !exactArray(result.deployments, 100)) throw new Error("private tester baseline reader unavailable");
    const current = result.deployments[0], currentRecord = current as RecordValue, currentVersions = currentRecord?.versions;
    if (!current || typeof current !== "object" || Array.isArray(current) || !text(currentRecord.id, UUID) || currentRecord.strategy !== "percentage" || !exactArray(currentVersions, 2) || currentVersions.length !== 1) throw new Error("private tester baseline reader unavailable");
    const version = currentVersions[0];
    if (!exactRecord(version, ["percentage", "version_id"]) || version.percentage !== 100 || !text(version.version_id, UUID)) throw new Error("private tester baseline reader unavailable");
    const observedAt = now();
    if (!Number.isSafeInteger(observedAt)) throw new Error("private tester baseline reader unavailable");
    return { provider: "cloudflare-api", identity: `account:${accountId}`, observedAt, body: { scriptName: SITES_WORKER_SCRIPT, deploymentId: (current as RecordValue).id, versionId: version.version_id, percentage: 100 } };
  } };
}
...
async function main(): Promise<void> {
  const [releasePath, manifestPath, receiptPath, outputPath] = process.argv.slice(2);
  if (!releasePath || !manifestPath || !receiptPath || !outputPath || process.argv.slice(2).length !== 4) throw new Error("private tester baseline configuration missing");
  const [releaseRaw, receiptRaw, deploymentManifest, reviewed] = await Promise.all([readFile(releasePath, "utf8"), readFile(receiptPath, "utf8"), readDeploymentManifestFile(manifestPath), verifyPrivateTesterD1SourceBaseline()]);
  const release = JSON.parse(releaseRaw), sitesDeploymentReceipt = JSON.parse(receiptRaw), expectedSitesDeploymentReceiptHash = envId(process.env, "SITES_DEPLOYMENT_RECEIPT_SHA256", HASH), project = envId(process.env, "KMS_PROJECT", /^[a-z][a-z0-9-]{2,62}$/), location = envId(process.env, "KMS_LOCATION", /^[A-Za-z0-9_-]{1,255}$/), keyRing = envId(process.env, "KMS_KEY_RING", /^[A-Za-z0-9_-]{1,255}$/), key = envId(process.env, "KMS_KEY", /^[A-Za-z0-9_-]{1,255}$/), principal = envId(process.env, "EVIDENCE_PRINCIPAL", /^[A-Za-z0-9_:/.@-]{3,200}$/), keyId = envId(process.env, "EVIDENCE_KEY_ID", /^[A-Za-z0-9_:/.@-]{3,200}$/), trustRaw = process.env.EVIDENCE_TRUST_JSON;
