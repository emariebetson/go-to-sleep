...
const POSTGRES_READER_USER = "nearyou-pt-baseline@nearnight.iam";
...
type SitesDeploymentReceipt = { version: 1; provider: "openai-sites-control-plane"; projectId: string; deploymentId: string; versionId: string; commitSha: string; deployedAt: string };
type Meta = { provider: string; identity: string; observedAt: number; rayId?: string };
type Observed = { provider: string; identity: string; observedAt: number; rayId?: string; body: unknown };
...
export type PrivateTesterBaseline = {
  version: 1; reviewRequired: true; capturedAt: number; release: PrivateTesterRelease;
  sites: { projectId: string; current: { version: string; commitSha: string }; rollback: { version: string; commitSha: string }; resources: ResourceIdentity[]; deployment: SitesDeploymentReceipt; buildReceipt: RecordValue };
...
export type PrivateTesterBaselineInput = { release: unknown; deploymentManifest: unknown; deploymentVerification: DeploymentVerification; sitesDeploymentReceipt: unknown; expectedSitesDeploymentReceiptHash: unknown; sitesResourceReceiptRaw:string; expectedSitesResourceReceiptHash:string; sitesEvidence: unknown; expectedD1Ledger: unknown; expectedD1SourceHash: unknown; expectedD1SchemaDefinitionHash: unknown; expectedD1SchemaObjectCount: unknown; outputPath: string; now(): number; readers: Readers };
...
function observation(value: unknown, nowMs: number): { meta: Meta; body: unknown } {
  const withRay=exactRecord(value,["provider","identity","observedAt","rayId","body"]),withoutRay=exactRecord(value,["provider","identity","observedAt","body"]),observedAt=withRay||withoutRay?value.observedAt:undefined;if((!withRay&&!withoutRay)||!text(value.provider,NAME)||!text(value.identity)||!Number.isSafeInteger(observedAt)||Number(observedAt)<nowMs-MAX_AGE_MS||Number(observedAt)>nowMs+MAX_FUTURE_MS||(withRay&&!text(value.rayId,/^[a-f0-9]{16}-[A-Z]{3}$/)))throw new Error("private tester baseline invalid");
  return { meta: { provider: value.provider, identity: value.identity, observedAt: Number(observedAt),...(withRay?{rayId:String(value.rayId)}:{}) }, body: value.body };
}
function sitesDeploymentReceipt(value: unknown, expectedHash: unknown, nowMs: number): SitesDeploymentReceipt { if(!exactRecord(value,["version","provider","projectId","deploymentId","versionId","commitSha","deployedAt"])||value.version!==1||value.provider!=="openai-sites-control-plane"||!text(value.projectId,/^appgprj_[a-f0-9]{32}$/)||!text(value.deploymentId,/^[A-Za-z0-9_-]{8,160}$/)||!text(value.versionId,/^appgprj_[a-f0-9]{32}~appgver_[A-Za-z0-9_-]{3,160}$/)||!text(value.commitSha,/^[a-f0-9]{40}$/)||typeof value.deployedAt!=="string"||typeof expectedHash!=="string"||!HASH.test(expectedHash)||hash(value)!==expectedHash)throw new Error("private tester baseline invalid");const deployedAt=Date.parse(value.deployedAt);if(!Number.isFinite(deployedAt)||deployedAt>nowMs+MAX_FUTURE_MS)throw new Error("private tester baseline invalid");return value as unknown as SitesDeploymentReceipt; }
...
export async function capturePrivateTesterBaseline(input: PrivateTesterBaselineInput): Promise<PrivateTesterBaseline> {
  ...
  let resourceEvidence: ReturnType<typeof parseSitesManagedResourceReceiptV3>; try { resourceEvidence=parseSitesManagedResourceReceiptV3(input.sitesResourceReceiptRaw,input.expectedSitesResourceReceiptHash,startedAt,input.sitesEvidence); } catch { throw new Error("private tester baseline invalid"); }
  const resourceReceipt=resourceEvidence.receipt as RecordValue,resourceVersion=resourceReceipt.version as RecordValue,resourceDeployment=resourceReceipt.deployment as RecordValue,buildReceipt=resourceEvidence.buildReceipt as RecordValue;
  if (manifest.schemaVersion!==3||manifest.releaseId !== release.releaseId || manifest.live.version !== release.sitesVersion || manifest.live.commitSha !== release.commitSha || deployment.projectId!==manifest.projectId || deployment.versionId!==manifest.live.version || deployment.commitSha!==manifest.live.commitSha || Math.abs(Date.parse(deployment.deployedAt)-manifest.issuedAt)>MAX_AGE_MS||resourceVersion.project_id!==manifest.projectId||resourceVersion.id!==manifest.live.version||(resourceVersion.source as RecordValue).commit_sha!==manifest.live.commitSha||resourceDeployment.id!==deployment.deploymentId||canonical(resourceEvidence.resources)!==canonical(manifest.resources)) throw new Error("private tester baseline invalid");
  const signedManifest = canonicalPrivateTesterDeploymentClaims(manifest);
  const raw = await Promise.all([read.d1.readLedger(), read.d1.readSchema(), read.postgres.readMigrations(), read.postgres.readCatalog(), read.dns.readIdentifiers(), read.oauth.readIdentifiers(), read.secretManager.listVersions(), read.gates.read()]);
  ...
  const d1LedgerRaw = observation(raw[0], capturedAt), d1SchemaRaw = observation(raw[1], capturedAt), pgMigrationsRaw = observation(raw[2], capturedAt), pgCatalogRaw = observation(raw[3], capturedAt), dnsRaw = observation(raw[4], capturedAt), oauthRaw = observation(raw[5], capturedAt), versionsRaw = observation(raw[6], capturedAt), gatesRaw = observation(raw[7], capturedAt),providerMeta={provider:"openai-sites-control-plane",identity:`project:${manifest.projectId}`,observedAt:Number(resourceReceipt.captured_at)};
  if (d1LedgerRaw.meta.provider !== "sites-runtime" || d1SchemaRaw.meta.provider !== "sites-runtime" || oauthRaw.meta.provider !== "sites-runtime" || gatesRaw.meta.provider !== "sites-runtime" || pgMigrationsRaw.meta.provider !== "cloud-sql" || pgCatalogRaw.meta.provider !== "cloud-sql" || pgMigrationsRaw.meta.identity !== POSTGRES_READER_IDENTITY || pgCatalogRaw.meta.identity !== POSTGRES_READER_IDENTITY) throw new Error("private tester baseline invalid");
  const runtimeRays=[d1LedgerRaw.meta.rayId,d1SchemaRaw.meta.rayId,gatesRaw.meta.rayId,oauthRaw.meta.rayId];if(runtimeRays.some(value=>typeof value!=="string")||new Set(runtimeRays).size!==4||d1SchemaRaw.meta.identity !== d1LedgerRaw.meta.identity || oauthRaw.meta.identity !== d1LedgerRaw.meta.identity || gatesRaw.meta.identity !== d1LedgerRaw.meta.identity) throw new Error("private tester baseline invalid");
  const applied = d1Applied(d1LedgerRaw.body, expected), schemaBody = d1Schema(d1SchemaRaw.body, input.expectedD1SchemaDefinitionHash, Number(input.expectedD1SchemaObjectCount)), migrations = ledger(exactRecord(pgMigrationsRaw.body, ["ledger"]) ? pgMigrationsRaw.body.ledger : undefined), catalog = pgCatalog(pgCatalogRaw.body), resources = manifest.resources, dnsBody = dns(dnsRaw.body), oauthBody = oauth(oauthRaw.body), secretVersions = versions(versionsRaw.body), gateBody = gates(gatesRaw.body);
  const observations = { d1Ledger: d1LedgerRaw.meta, d1Schema: d1SchemaRaw.meta, postgresMigrations: pgMigrationsRaw.meta, postgresCatalog: pgCatalogRaw.meta, sitesResourceReceipt:providerMeta, dns: dnsRaw.meta, oauth: oauthRaw.meta, secretManager: versionsRaw.meta, gates: gatesRaw.meta };
  const baseline: PrivateTesterBaseline = { version: 1, reviewRequired:true, capturedAt, release, sites: { projectId: manifest.projectId, current: manifest.live, rollback: manifest.rollback, resources, deployment, buildReceipt }, d1: { appliedMigrations: applied, appliedLedgerHash: hash(applied), reviewedSourceHash: input.expectedD1SourceHash, schemaHash: hash(schemaBody.objects), schemaObjectCount: schemaBody.objects.length, sourceSchemaDefinitionHash: schemaBody.definitionHash, sourceSchemaObjectCount: schemaBody.sourceObjects.length, providerInternalSchemaHash: hash(schemaBody.providerInternalObjects), providerInternalSchemaObjectCount: schemaBody.providerInternalObjects.length }, postgres: { migrationsHash: hash(migrations), catalogHash: hash(catalog) }, dns: dnsBody, oauth: oauthBody, bindings: { bindings: resources }, secretVersions, gates: gateBody, observations };
  validatePrivateTesterBaselineCandidate(baseline,capturedAt);
...
function sitesGateway(subject: string, expectedRelease: PrivateTesterRelease, fetcher: typeof fetch, now: () => number) { const principal=`service:${subject}`,endpoint=new URL(SITES_GATEWAY_ORIGIN);return async(kind:string):Promise<Observed>=>{const response=await fetcher(new URL(`/api/internal/private-tester-baseline/${encodeURIComponent(kind)}`,endpoint),{headers:{authorization:`Bearer ${await gatewayToken(fetcher)}`,accept:"application/json"},redirect:"error",signal:AbortSignal.timeout(10_000)}),rayId=response.headers.get("cf-ray")??"",raw=await response.text(),completedAt=now();let value:unknown;try{value=JSON.parse(raw)}catch{throw new Error("private tester baseline reader unavailable")}if(!response.ok||!text(rayId,/^[a-f0-9]{16}-[A-Z]{3}$/)||!exactRecord(value,["issuer","audience","subject","principal","observedAt","release","body"])||value.issuer!==GOOGLE_ISSUER||value.audience!==SITES_GATEWAY_ORIGIN||value.subject!==subject||value.principal!==principal||canonical(value.release)!==canonical(expectedRelease)||!Number.isSafeInteger(completedAt)||!Number.isSafeInteger(value.observedAt)||Number(value.observedAt)<completedAt-MAX_AGE_MS||Number(value.observedAt)>completedAt+MAX_FUTURE_MS)throw new Error("private tester baseline reader unavailable");return{provider:"sites-runtime",identity:principal,observedAt:Number(value.observedAt),rayId,body:value.body}};}
function postgresConnection(environment: NodeJS.ProcessEnv): string { const raw = environment.PRIVATE_TESTER_BASELINE_DATABASE_URL ?? "", marker = environment.CLOUD_SQL_IAM_CONNECTOR ?? "", instance = environment.CLOUD_SQL_INSTANCE_CONNECTION_NAME ?? "", expected = environment.CLOUD_SQL_PROXY_ARGS_CHECKSUM ?? "", template = readFileSync(new URL("../infra/production/cloud-sql-auth-proxy.args", import.meta.url), "utf8"), artifact = template.replace("${CLOUD_SQL_INSTANCE_CONNECTION_NAME}", instance), checksum = createHash("sha256")["up\u0064ate"](artifact).digest("hex"); let url: URL; try { url = new URL(raw); } catch { throw new Error("private tester baseline configuration missing"); } if (marker !== "cloud-sql-auth-proxy" || instance !== "nearnight:us-central1:nearyou-production" || expected !== checksum || !artifact.includes("--auto-iam-authn") || !artifact.endsWith(`${instance}\n`) || !["postgres:", "postgresql:"].includes(url.protocol) || url.password || url.hostname !== "127.0.0.1" || url.port !== "5432" || url.searchParams.get("sslmode") !== "disable") throw new Error("private tester baseline configuration missing"); return raw; }
function postgresReaders(environment: NodeJS.ProcessEnv) { const connectionString = postgresConnection(environment), expectedUser = envId(environment, "NEARYOU_PRIVATE_TESTER_BASELINE_DATABASE_USER", /^nearyou-pt-baseline@nearnight\.iam$/); const query = async <T extends RecordValue>(sql: string) => { const name = "pg", { Pool } = await import(name) as typeof import("pg"), pool = new Pool({ connectionString, ssl: { rejectUnauthorized: false } }); try { const statement = sql.trim().replace(/;$/, ""), rows = (await pool.query<T>(statement)).rows, completion = (await pool.query<{ observed_at: string; session_identity: string; current_identity: string; database_name: string; principal: string; effective: boolean }>("SELECT floor(extract(epoch from clock_timestamp())*1000)::bigint::text AS observed_at,session_user::text AS session_identity,current_user::text AS current_identity,a.database_name,a.principal,a.effective FROM nearyou.assert_private_tester_baseline_verifier() a")).rows[0], observedAt = Number(completion?.observed_at); if (!Number.isSafeInteger(observedAt) || rows.length < 1 || completion?.session_identity !== expectedUser || completion.current_identity !== expectedUser || completion.database_name !== "nearyou" || completion.effective !== true || completion.principal !== "service:nearyou-private-tester-baseline-verifier" || expectedUser !== POSTGRES_READER_USER) throw new Error("private tester baseline reader unavailable"); return { observedAt, identity: `database:${completion.session_identity}`, rows }; } finally { await pool.end(); } };
...
async function readBoundedNoFollow(inputPath:string,maxBytes:number):Promise<string>{if(!inputPath||inputPath.length>4_096)throw new Error("private tester baseline configuration missing");let handle;try{handle=await open(inputPath,constants.O_RDONLY|constants.O_NOFOLLOW);const stat=await handle.stat();if(!stat.isFile()||stat.size<2||stat.size>maxBytes)throw new Error();const raw=await handle.readFile("utf8");if(Buffer.byteLength(raw)!==stat.size)throw new Error();return raw}catch{throw new Error("private tester baseline configuration missing")}finally{await handle?.close()}}
async function main(): Promise<void> {
  const [releasePath, manifestPath, receiptPath,resourceReceiptPath, evidencePath, outputPath] = process.argv.slice(2);
  if (!releasePath || !manifestPath || !receiptPath ||!resourceReceiptPath|| !evidencePath || !outputPath || process.argv.slice(2).length !== 6) throw new Error("private tester baseline configuration missing");
  const [releaseRaw, receiptRaw,sitesResourceReceiptRaw,evidenceRaw, deploymentManifest, reviewed] = await Promise.all([readFile(releasePath, "utf8"), readFile(receiptPath, "utf8"),readBoundedNoFollow(resourceReceiptPath,1_048_576),readBoundedNoFollow(evidencePath,1_048_576), readDeploymentManifestFile(manifestPath), verifyPrivateTesterD1LiveState()]);
  let sitesEvidence: unknown; try { sitesEvidence = JSON.parse(evidenceRaw); } catch { throw new Error("private tester baseline configuration missing"); }
