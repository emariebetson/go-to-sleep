...
type SitesDeploymentReceipt = { version: 1; provider: "openai-sites-control-plane"; projectId: string; deploymentId: string; versionId: string; commitSha: string; deployedAt: string; workerDeploymentId: string; workerVersionId: string };
...
export type PrivateTesterBaseline = {
  ...
  sites: { projectId: string; current: { version: string; commitSha: string }; rollback: { version: string; commitSha: string }; resources: ResourceIdentity[]; deployment: SitesDeploymentReceipt };
...
export type PrivateTesterBaselineInput = { release: unknown; deploymentManifest: unknown; deploymentVerification: DeploymentVerification; sitesDeploymentReceipt: unknown; expectedSitesDeploymentReceiptHash: unknown; sitesResourceReceiptRaw:string; expectedSitesResourceReceiptHash:string; expectedD1Ledger: unknown; expectedD1SourceHash: unknown; expectedD1SchemaDefinitionHash: unknown; expectedD1SchemaObjectCount: unknown; outputPath: string; now(): number; readers: Readers };
...
function sitesDeploymentReceipt(value: unknown, expectedHash: unknown, nowMs: number): SitesDeploymentReceipt { if(!exactRecord(value,["version","provider","projectId","deploymentId","versionId","commitSha","deployedAt","workerDeploymentId","workerVersionId"])||value.version!==1||value.provider!=="openai-sites-control-plane"||!text(value.projectId,/^appgprj_[a-f0-9]{32}$/)||!text(value.deploymentId,/^[A-Za-z0-9_-]{8,160}$/)||!text(value.versionId,/^appgprj_[a-f0-9]{32}~appgver_[A-Za-z0-9_-]{3,160}$/)||!text(value.commitSha,/^[a-f0-9]{40}$/)||typeof value.deployedAt!=="string"||!text(value.workerDeploymentId,UUID)||!text(value.workerVersionId,UUID)||typeof expectedHash!=="string"||!HASH.test(expectedHash)||hash(value)!==expectedHash)throw new Error("private tester baseline invalid");const deployedAt=Date.parse(value.deployedAt);if(!Number.isFinite(deployedAt)||deployedAt>nowMs+MAX_FUTURE_MS)throw new Error("private tester baseline invalid");return value as unknown as SitesDeploymentReceipt; }
...
export async function capturePrivateTesterBaseline(input: PrivateTesterBaselineInput): Promise<PrivateTesterBaseline> {
  ...
  const resourceEvidence=parseSitesManagedResourceReceipt(input.sitesResourceReceiptRaw,input.expectedSitesResourceReceiptHash,startedAt),resourceReceipt=resourceEvidence.receipt as RecordValue,resourceVersion=resourceReceipt.version as RecordValue,resourceDeployment=resourceReceipt.deployment as RecordValue;
  if (manifest.schemaVersion!==2||manifest.releaseId !== release.releaseId || manifest.live.version !== release.sitesVersion || manifest.live.commitSha !== release.commitSha || deployment.projectId!==manifest.projectId || deployment.versionId!==manifest.live.version || deployment.commitSha!==manifest.live.commitSha || Math.abs(Date.parse(deployment.deployedAt)-manifest.issuedAt)>MAX_AGE_MS||resourceVersion.project_id!==manifest.projectId||resourceVersion.id!==manifest.live.version||(resourceVersion.source as RecordValue).commit_sha!==manifest.live.commitSha||resourceDeployment.id!==deployment.deploymentId||canonical(resourceEvidence.resources)!==canonical(manifest.resources)) throw new Error("private tester baseline invalid");
