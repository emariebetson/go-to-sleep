...
### Task 2: Durable over-capacity state and atomic enforcement
...
- [ ] **Step 3: Implement migration and schema**
  ...
  Add the table, immutable version/CAS checks, exact JSON generated by SQL, and triggers that recalculate after entitlement and usage changes. Extend existing BEFORE triggers so capacity-consuming operations require the projected post-mutation usage to be within the current plan; do not block DELETE, consent revocation, export, billing, or member departure.
...
**Interfaces:**
- Consumes: authenticated household context, PostgreSQL rollout authorization, D1 entitlement/capacity state.
- Produces:
  ```ts
...
- [ ] **Step 3: Implement the read service and route branch**
  ...
  Keep the literal-off guard as the first executable statement. In the unreachable enabled branch, require authenticated household context, exact PostgreSQL `nearfamily` authorization, effective Family entitlement, and current D1 capacity state before returning `Cache-Control: no-store` JSON.
