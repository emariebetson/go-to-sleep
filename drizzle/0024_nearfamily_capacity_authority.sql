...
BEGIN SELECT RAISE(ABORT,'household_capacity_restricted'); END;
--> statement-breakpoint
...
BEGIN SELECT CASE WHEN (SELECT `children`>=`child_limit` FROM `household_capacity_projection` WHERE `household_id`=NEW.`household_id`) THEN RAISE(ABORT,'household_child_limit_reached') END; END;
--> statement-breakpoint
...
BEGIN SELECT CASE WHEN (SELECT `voices`>=`voice_limit` FROM `household_capacity_projection` WHERE `household_id`=NEW.`household_id`) THEN RAISE(ABORT,'household_voice_limit_reached') END; END;
