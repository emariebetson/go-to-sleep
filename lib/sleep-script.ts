...
export function validateScriptInput(input: Partial<ScriptInput>): ScriptInput {
  ...
  return { childName, ageMonths: String(age), challenge: input.challenge!, theme: input.theme!, duration: input.duration!, style: input.style!, scriptMode: input.scriptMode!, contentType: input.contentType!, sourceUrl };
}
...
export async function personalizedScript(input: ScriptInput) {
  ...
  if (!apiKey) return fallbackPersonalizedScript(input);
  ...
  const sourcePrompt = input.source
    ? `\nOptional YouTube inspiration: title "${input.source.title}" by "${input.source.creator || "unknown creator"}". Use only the broad mood or high-level premise suggested by this metadata. Create fully original wording and events.`
    : "";
  const prompt = `Bedtime type: ${labels[input.contentType]}\nBaby nickname: ${input.childName}\nAge: ${input.ageMonths} months\nBedtime context: ${labels[input.challenge]}\nStory world: ${labels[input.theme]}\nNarration style: ${labels[input.style]}\nRequested duration: ${input.duration} minutes${sourcePrompt}`;
  ...
  if (!response.ok) {
    ...
    return fallbackPersonalizedScript(input);
  }
  const payload = await response.json() as { output_text?: string; output?: Array<{ content?: Array<{ type?: string; text?: string }> }> };
  const text = payload.output_text || payload.output?.flatMap((item) => item.content || []).find((item) => item.type === "output_text")?.text;
  if (!text?.trim()) return fallbackPersonalizedScript(input);
  return text.trim();
}
