...
export type Claims = { version: 1; principal: string; keyId: string; keyVersion: number; releaseId: string; schema: string; backfill: string; highWater: number; fence: number; notBefore: number; issuedAt: number; expiresAt: number; nonce: string; gates: Record<Kind, Gate>; shadow: { kind: "shadow"; policyVersion: 1; releaseId: string; schema: string; artifact: string; startedAt: number; endedAt: number; sourceChecksum: string; targetChecksum: string; sampleCount: number; observedRows:number; mismatchCount: 0 } };
