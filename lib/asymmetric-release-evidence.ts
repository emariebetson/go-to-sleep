...
type Kind = "rls" | "media" | "restore" | "load" | "accessibility" | "security";
...
export type Claims = { version: 1; principal: string; keyId: string; keyVersion: number; releaseId: string; schema: string; backfill: string; highWater: number; fence: number; notBefore: number; issuedAt: number; expiresAt: number; nonce: string; gates: Record<Kind, Gate>; shadow: { kind: "shadow"; policyVersion: 1; releaseId: string; schema: string; artifact: string; startedAt: number; endedAt: number; sourceChecksum: string; targetChecksum: string; sampleCount: number; mismatchCount: 0 } };
...
function validateResults(kind: Kind, value: unknown) {
  ...
  exact(v, ["critical", "high", "scanArtifact", "penTestArtifact"]); return v.critical === 0 && v.high === 0 && typeof v.scanArtifact === "string" && HASH.test(v.scanArtifact) && typeof v.penTestArtifact === "string" && HASH.test(v.penTestArtifact);
}
function validateClaims(claims: Claims) {
  exact(claims, ["version", "principal", "keyId", "keyVersion", "releaseId", "schema", "backfill", "highWater", "fence", "notBefore", "issuedAt", "expiresAt", "nonce", "gates", "shadow"]);
  ...
  exact(claims.gates, ["rls", "media", "restore", "load", "accessibility", "security"]);
