...
import { entitlements, householdStorageReservations, jobs, mediaAssets, operationalOutcomeOutbox, providerSpendReservations, storyExperiences, storyMediaBindings, storyPersistStagingObjects, storyProviderBudgetHolds, storySegments, storySoundAssets, storyWorkerCheckpoints, task2cMediaIntegrity, usageReservations, voiceConsentLeases, voiceConsents, voices } from "@/db/schema";
...
import { outcomeChecksum, type OutcomeRecord } from "./operational-outcome-outbox";
...
export function createNearStoryProductionDependencies() {
  ...
  const deps: NearStoryWorkerDependencies = {
    ...
    async complete(work, result) {
      const now = new Date(),owned=await getDb().select({requestHash:jobs.requestHash,releaseId:jobs.rolloutReleaseId,releaseVersion:jobs.rolloutVersion}).from(jobs).where(and(eq(jobs.id,work.jobId),eq(jobs.householdId,work.householdId),eq(jobs.status,"running"),eq(jobs.workerAttemptToken,work.attemptToken||""))).get();if(!owned?.releaseId||!owned.releaseVersion||!work.attemptToken)throw new StoryPersistenceUncertainError();const outcome:OutcomeRecord={product:"nearstory",operation:"terminal",jobId:work.jobId,householdId:work.householdId,attemptToken:work.attemptToken,requestHash:owned.requestHash,releaseId:owned.releaseId,releaseVersion:owned.releaseVersion,terminalStatus:"succeeded"},checksum=await outcomeChecksum(outcome),outboxId=`nearstory:${work.jobId}:${work.attemptToken}:terminal`; const receipts = await getDb().batch([
        ...
        getDb().insert(operationalOutcomeOutbox).values({id:outboxId,product:"nearstory",operation:"terminal",jobId:work.jobId,householdId:work.householdId,attemptToken:work.attemptToken,requestHash:owned.requestHash,releaseId:owned.releaseId,releaseVersion:owned.releaseVersion,terminalStatus:"succeeded",deliveryStatus:"pending",attempts:0,nextAttemptAt:now,payloadChecksum:checksum,createdAt:now,updatedAt:now}).onConflictDoNothing().returning({id:operationalOutcomeOutbox.id}),
      ]);
      if (receipts.some((receipt) => receipt.length !== 1)) throw new StoryPersistenceUncertainError();
    },
    async fail(work, code) {const now=new Date(),owned=await getDb().select({requestHash:jobs.requestHash,releaseId:jobs.rolloutReleaseId,releaseVersion:jobs.rolloutVersion}).from(jobs).where(and(eq(jobs.id,work.jobId),eq(jobs.householdId,work.householdId),eq(jobs.status,"running"),eq(jobs.workerAttemptToken,work.attemptToken||""))).get();if(!owned?.releaseId||!owned.releaseVersion||!work.attemptToken)throw new StoryPersistenceUncertainError();const outcome:OutcomeRecord={product:"nearstory",operation:"terminal",jobId:work.jobId,householdId:work.householdId,attemptToken:work.attemptToken,requestHash:owned.requestHash,releaseId:owned.releaseId,releaseVersion:owned.releaseVersion,terminalStatus:"failed"},checksum=await outcomeChecksum(outcome);await getDb().batch([getDb().update(storyExperiences).set({ status: "failed", errorCode: code, updatedAt: now }).where(and(eq(storyExperiences.id, work.storyId), eq(storyExperiences.householdId, work.householdId), inArray(storyExperiences.status, ["queued", "processing", "review_required"]))), getDb().update(jobs).set({ status: "failed", errorCode: code, progressStage: "failed", workerAttemptToken: null, workerLeaseExpiresAt: null, completedAt: now, updatedAt: now }).where(and(eq(jobs.id, work.jobId), eq(jobs.householdId, work.householdId), eq(jobs.status, "running"), eq(jobs.workerAttemptToken, work.attemptToken))),getDb().insert(operationalOutcomeOutbox).values({id:`nearstory:${work.jobId}:${work.attemptToken}:terminal`,product:"nearstory",operation:"terminal",jobId:work.jobId,householdId:work.householdId,attemptToken:work.attemptToken,requestHash:owned.requestHash,releaseId:owned.releaseId,releaseVersion:owned.releaseVersion,terminalStatus:"failed",deliveryStatus:"pending",attempts:0,nextAttemptAt:now,payloadChecksum:checksum,createdAt:now,updatedAt:now}).onConflictDoNothing()]); },
  };
  return deps;
}
