...
const crcBytes=(value:Uint8Array)=>{let crc=0xffffffff;for(const byte of value){crc^=byte;for(let bit=0;bit<8;bit+=1)crc=(crc>>>1)^(0x82f63b78&-(crc&1))}return String((crc^0xffffffff)>>>0)};
export class CloudKmsEvidenceSigner{
 constructor(private options:{versionedKeyName:string;accessToken():Promise<string>;fetch?:typeof fetch}){if(!/^projects\/[a-z0-9-]+\/locations\/[A-Za-z0-9_-]+\/keyRings\/[A-Za-z0-9_-]+\/cryptoKeys\/[A-Za-z0-9_-]+\/cryptoKeyVersions\/[1-9][0-9]*$/.test(options.versionedKeyName))throw new Error("KMS signer invalid")}
 async sign(canonicalClaims:string){const data=new TextEncoder().encode(canonicalClaims);if(data.byteLength<2||data.byteLength>256*1024)throw new Error("KMS signer invalid");const digestBytes=new Uint8Array(await crypto.subtle.digest("SHA-256",data)),digest= btoa(String.fromCharCode(...digestBytes)),token=await this.options.accessToken();if(!/^[A-Za-z0-9._~-]{20,4096}$/.test(token))throw new Error("KMS identity failed");let response:Response;try{response=await(this.options.fetch??fetch)(`https://cloudkms.googleapis.com/v1/${this.options.versionedKeyName}:asymmetricSign`,{method:"POST",redirect:"error",headers:{authorization:`Bearer ${token}`,"content-type":"application/json"},body:JSON.stringify({digest:{sha256:digest},digestCrc32c:crcBytes(digestBytes)})})}catch{throw new Error("KMS signing failed")}const text=await response.text();if(!response.ok||text.length>65536)throw new Error("KMS signing failed");let value:Record<string,unknown>;try{value=JSON.parse(text)}catch{throw new Error("KMS signing failed")}if(value.name!==this.options.versionedKeyName||typeof value.signature!=="string"||typeof value.signatureCrc32c!=="string")throw new Error("KMS signing failed");let signature:Uint8Array;try{signature=Uint8Array.from(atob(value.signature),v=>v.charCodeAt(0))}catch{throw new Error("KMS signing failed")}if(signature.byteLength!==384||value.signatureCrc32c!==crcBytes(signature)||value.verifiedDigestCrc32c!==true)throw new Error("KMS signing failed");return value.signature.replace(/=/g,"").replace(/\+/g,"-").replace(/\//g,"_")}
}
