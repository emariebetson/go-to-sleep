...
test("disposable catalog candidates remain review artifacts but cannot promote migration head 0007",async()=>{const directory=await mkdtemp(join(tmpdir(),"nearyou-catalog-")),output=join(directory,"catalog-manifest.candidate.json"),rows=REQUIRED_CATALOG_KINDS.map((kind,index)=>({kind,identity:`nearyou.${kind}.${index}`,definition:`definition-${index}`})),connect=async()=>({query:async(sql)=>({rows:sql.includes("public_execute_count")?[{forced_rls:["household_members","tenant_records"],public_execute_count:"0"}]:rows}),close:async()=>{}}),candidate=await generateCatalogCandidate({databaseUrl:"postgres://fixture",output,connect});try{assert.equal(candidate.migrationHead,"0007_private_tester_deployment_manifest");await assert.rejects(()=>promoteCatalogManifest({candidate:output,output:join(directory,"catalog-manifest.reviewed.json")}),/promotion/)}finally{await rm(directory,{recursive:true,force:true})}});
