...
import { canonicalPolicyDefinition, LIVE_CATALOG_QUERY, LIVE_CATALOG_SECURITY_QUERY, validateCatalogRows } from "../scripts/postgres-catalog.ts";
...
test("disposable catalog candidates remain review artifacts but cannot promote the current migration head",async()=>{const directory=await mkdtemp(join(tmpdir(),"nearyou-catalog-")),output=join(directory,"catalog-manifest.candidate.json"),rows=REQUIRED_CATALOG_KINDS.map((kind,index)=>({kind,identity:`nearyou.${kind}.${index}`,definition:`definition-${index}`})),connect=async()=>({query:async(sql)=>({rows:sql.includes("public_execute_count")?[{forced_rls:["household_members","tenant_records","private_tester_activation_baselines","private_tester_activation_state","private_tester_activation_invites","private_tester_activation_audit","nearfamily_decision_nonces"],public_execute_count:"0"}]:rows}),close:async()=>{}}),candidate=await generateCatalogCandidate({databaseUrl:"postgres://fixture",output,connect});try{assert.equal(candidate.migrationHead,"0013_nearfamily_decision_nonce_and_evidence");await assert.rejects(()=>promoteCatalogManifest({candidate:output,output:join(directory,"catalog-manifest.reviewed.json")}),/promotion/)}finally{await rm(directory,{recursive:true,force:true})}});
...
test("catalog CLI reports only bounded failure classes",()=>{
  ...
  assert.equal(catalogCandidateFailureCode(new Error("catalog security invariant failed:forced-rls:household_members")),"catalog-forced-rls:household_members");
  ...
  assert.equal(catalogCandidateFailureCode(new Error("catalog security invariant failed:public-execute:unknown")),"catalog-public-execute:unknown");
...
test("catalog candidate rejects missing FORCE RLS and PUBLIC function execution",async()=>{const rows=REQUIRED_CATALOG_KINDS.map((kind,index)=>({kind,identity:`i${index}`,definition:`d${index}`})),directory=await mkdtemp(join(tmpdir(),"nearyou-catalog-security-"));try{for(const security of [{forced_rls:["household_members"],public_execute_count:"0"},{forced_rls:["household_members","tenant_records","private_tester_activation_baselines","private_tester_activation_state","private_tester_activation_invites","private_tester_activation_audit"],public_execute_count:"1"}])await assert.rejects(()=>generateCatalogCandidate({databaseUrl:"postgres://fixture",output:join(directory,`${Math.random()}-catalog-manifest.candidate.json`),connect:async()=>({query:async sql=>({rows:sql.includes("public_execute_count")?[security]:rows}),close:async()=>{}})}),/security invariant/)}finally{await rm(directory,{recursive:true,force:true})}});
