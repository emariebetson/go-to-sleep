...
async function fixture(overrides = {}) {
  ...
  const query = async (sql, args = []) => {
    ...
    if (sql.includes("public_execute_count")) return { rows: [{ forced_rls: ledger.length >= 13 ? ["household_members", "tenant_records", "private_tester_activation_baselines", "private_tester_activation_state", "private_tester_activation_invites", "private_tester_activation_audit", "nearfamily_decision_nonces"] : ["household_members", "tenant_records"], public_execute_count: "0" }] };
...
test("prepares a review-required catalog from exact live PostgreSQL 16 state and records immutable provenance", async () => {
  ...
  assert.equal(result.candidate.migrationHead, "0014_release_policy_evidence_read");
...
test("legacy 0001-0004 ledger is remediated through 0010 and preserved exactly in provenance", async () => {
  const migrations = await loadPostgresMigrations();
