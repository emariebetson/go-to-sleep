...
test("main evidence build avoids the staging bucket and retains pinned-builder digests",()=>{const workflow=readFileSync(new URL("../.github/workflows/production-evidence.yml",import.meta.url),"utf8");assert.doesNotMatch(workflow,/gcloud builds submit|auth configure-docker/);assert.match(workflow,/gcr\.io\/cloud-builders\/docker@sha256:[a-f0-9]{64}/);assert.match(workflow,/auth print-access-token \| docker run --rm -i[\s\S]*login -u oauth2accesstoken --password-stdin/);assert.match(workflow,/database-proof-image: \$\{\{ steps\.build-images\.outputs\.database_proof_image \}\}/);assert.match(workflow,/proof_image='\$\{\{ needs\.readiness-gateway-image\.outputs\.database-proof-image \}\}'/);assert.match(workflow,/readiness-gateway-image-build\.json/);assert.match(workflow,/image_summary\.fully_qualified_digest/)});
