...
test("captures fixed bounded convergence mutation projections as counts and hashes only",async()=>{const seen=[];const DB={prepare:(sql)=>({all:async()=>{seen.push(sql);if(!sql.includes("/* convergence_probe:"))throw new Error("unexpected query");return{results:sql.includes("preflight")?[{rowCount:3,violationCount:0}]:[{identity:"opaque-id",target:"opaque-target"}]}}})};const runtime=createPrivateTesterBaselineRuntime(runtimeEnvironment({DB}),{now:()=>now,expectedD1SchemaDefinitionHash:schemaDefinitionHash,expectedD1SchemaObjectCount:sourceSchemaRows.length,fetch});const value=await runtime.read("d1-convergence-probes");assert.equal(value.version,1);assert.equal(value.stages.length,13);assert.equal(value.stages.every(row=>JSON.stringify(Object.keys(row).sort())===JSON.stringify(["projectionSha256","rowCount","stage","violationCount"])),true);assert.equal(value.stages.every(row=>Number.isSafeInteger(row.rowCount)&&Number.isSafeInteger(row.violationCount)&&/^[a-f0-9]{64}$/.test(row.projectionSha256)),true);assert.equal(JSON.stringify(value).includes("opaque"),false);assert.equal(seen.length,13);assert.equal(seen.every(sql=>!sql.includes("?")),true)});
test("convergence probes fail closed on excess rows malformed counts or non-scalar values",async()=>{for(const results of[Array.from({length:1001},()=>({identity:"x"})),[{rowCount:1,violationCount:-1}],[{identity:"x",target:{secret:"forbidden"}}]]){const DB={prepare:(sql)=>({all:async()=>({results:sql.includes("preflight")&&results.length===1&&Object.hasOwn(results[0],"rowCount")?results:results})})};const runtime=createPrivateTesterBaselineRuntime(runtimeEnvironment({DB}),{now:()=>now,expectedD1SchemaDefinitionHash:schemaDefinitionHash,expectedD1SchemaObjectCount:sourceSchemaRows.length,fetch});await assert.rejects(()=>runtime.read("d1-convergence-probes"),/evidence unavailable/)}});
test("convergence probes use exact deletion and playing-queue semantics without constant placeholders",()=>{const source=readFileSync(new URL("../lib/private-tester-baseline-gateway.ts",import.meta.url),"utf8");assert.doesNotMatch(source,/convergence_probe:[^*]+\*\/\s*SELECT\s+0\b/i);assert.match(source,/FROM deletion_reconciliations d ORDER BY d\.id/);assert.doesNotMatch(source,/deletion_reconciliations d WHERE d\.household_id/);for(const table of["sleep_sessions","voices","voice_replacements","households"])assert.match(source,new RegExp(`FROM ${table}`));assert.match(source,/FROM bedtime_queue_items WHERE status='playing' GROUP BY household_id HAVING COUNT\(\*\)>1/);assert.match(source,/SUM\(playing_count-1\)/)});

test("all convergence probes execute against the exact source schema at head 0009",async()=>{const sqlite=new DatabaseSync(":memory:");for(const name of migrationNames.slice(0,10)){const sql=readFileSync(new URL(`../drizzle/${name}`,import.meta.url),"utf8").replaceAll("--> statement-breakpoint","");sqlite.exec(sql)}const DB={prepare:(sql)=>({all:async()=>({results:sqlite.prepare(sql).all().map(row=>({...row}))})})},runtime=createPrivateTesterBaselineRuntime(runtimeEnvironment({DB}),{now:()=>now,expectedD1SchemaDefinitionHash:schemaDefinitionHash,expectedD1SchemaObjectCount:sourceSchemaRows.length,fetch}),value=await runtime.read("d1-convergence-probes");assert.equal(value.stages.length,13);sqlite.close()});
test("storage insert and commit affected sets both exclude invalid ready media",async()=>{const sqlite=new DatabaseSync(":memory:");sqlite.exec("CREATE TABLE media_assets(id TEXT,household_id TEXT,byte_size INTEGER,status TEXT,created_at INTEGER,updated_at INTEGER);INSERT INTO media_assets VALUES('valid','h',10,'ready',1,2),('null','h',NULL,'ready',1,2),('zero','h',0,'ready',1,2)");const DB={prepare:(sql)=>({all:async()=>{if(sql.includes("convergence_probe:0012-storage-reservation-"))return{results:sqlite.prepare(sql).all().map(row=>({...row}))};return{results:sql.includes("preflight")?[{rowCount:0,violationCount:0}]:[]}}})};const runtime=createPrivateTesterBaselineRuntime(runtimeEnvironment({DB}),{now:()=>now,expectedD1SchemaDefinitionHash:schemaDefinitionHash,expectedD1SchemaObjectCount:sourceSchemaRows.length,fetch}),value=await runtime.read("d1-convergence-probes"),insert=value.stages.find(row=>row.stage==="0012-storage-reservation-insert"),commit=value.stages.find(row=>row.stage==="0012-storage-reservation-commit");assert.equal(insert.rowCount,1);assert.equal(commit.rowCount,1);assert.equal(insert.projectionSha256,createHash("sha256").update(JSON.stringify([{identity:"storage:valid",household_id:"h",media_asset_id:"valid",byte_size:10,target:"reserved",created_at:1,updated_at:2}])).digest("hex"));assert.equal(commit.projectionSha256,createHash("sha256").update(JSON.stringify([{identity:"valid",target:"committed"}])).digest("hex"))});
test("consent probe hashes the exact affected set and action without returning identities",async()=>{const seen=[];const DB={prepare:(sql)=>({all:async()=>{if(sql.includes("0011-consent-revocation")){seen.push(sql);return{results:[{identity:"consent-private",prior:null,target:"revoke"}]}}return{results:sql.includes("preflight")?[{rowCount:0,violationCount:0}]:[]}}})};const runtime=createPrivateTesterBaselineRuntime(runtimeEnvironment({DB}),{now:()=>now,expectedD1SchemaDefinitionHash:schemaDefinitionHash,expectedD1SchemaObjectCount:sourceSchemaRows.length,fetch}),value=await runtime.read("d1-convergence-probes"),stage=value.stages.find(row=>row.stage==="0011-consent-revocation");assert.equal(stage.rowCount,1);assert.equal(stage.projectionSha256,createHash("sha256").update(JSON.stringify([{identity:"consent-private",prior:null,target:"revoke"}])).digest("hex"));assert.equal(JSON.stringify(value).includes("consent-private"),false);assert.match(seen[0],/status='active_verified'.*current_consent_id/s)});
