...
BEGIN
  ...
  IF not_before_ms>issued_at_ms OR issued_at_ms>=expires_at_ms OR expires_at_ms-issued_at_ms>900000 OR not_before_ms>server_now_ms+30000 OR issued_at_ms>server_now_ms+30000 OR server_now_ms-issued_at_ms>300000 OR expires_at_ms<=server_now_ms OR expires_at_ms>server_now_ms+900000 OR expires_at_ms IS DISTINCT FROM floor(extract(epoch FROM p_expiry)*1000)::bigint THEN RETURN false; END IF;
